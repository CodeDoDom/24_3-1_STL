...
public:
	...
	istream& read(istream& is) {
		delete[] p;		// 만일 p가 nullptr이라면 아무 일도 일어나지 않는다.
...
//---------
{
	...
	cout << endl;
	cout << "[문제 3] 답" << endl;
	cout << "정렬 후 마지막 객체의 num: " << v.back().getNum() << endl;
	//cout << "정렬 후 마지막 객체의 num: " << v[v.size() - 1].getNum() << endl;
	//cout << "정렬 후 마지막 객체의 num: " << v.rbegin()->getNum() << endl;

	//--------------------------------------------------------------------------
	// [문제 4] [문제 3]의 정렬을 더 효율적으로 실행하고 싶다. 
	// 해결방안을 간단하게 적어라.										   (10)
	//			
	//			답: 자원을 관리하는 이 클래스가 이동문법을 지원하면 효율적이다.	
	//--------------------------------------------------------------------------
	...
	// 
	//						답: free store에 확보한 메모리를 해제하지 않고 있다.
	//							소멸자에서 자원을 반환하도록 한다.
	//--------------------------------------------------------------------------
